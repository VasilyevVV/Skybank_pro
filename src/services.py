...
def profit_categories(input_df: pd.DataFrame, calc_month: int, calc_year: int) -> json:
    ...
    if input_df.empty:
        ...
        if end_day == 0:
            ...
            # Отбор только успешных операций, со статусом, не равным FAILED, и только платежей (операция < 0.0)
...
def simple_search(search_str: str, input_tx: list[dict]) -> str:
    ...
    if input_tx:
        result = [
             ...
             if (isinstance(operation.get("Категория", ""), str) and
                re.search(search_str, operation["Категория"], flags=re.IGNORECASE))
                or
                (isinstance(operation.get("Описание", ""), str) and
                 re.search(search_str, operation["Описание"], flags=re.IGNORECASE))
...
#fp = str(os.path.join(TEST_FILE_DIR, "test_empty_tx.xlsx"))
# fp = str(os.path.join(TEST_FILE_DIR, "test_operations.xlsx"))
# txl = get_tx_list(fp)
# # print(txl)
#
# l1 = simple_search("Ситидрайв", txl)
# print(l1)